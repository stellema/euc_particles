# -*- coding: utf-8 -*-
"""
created: Tue May 19 15:50:47 2020
...
def set_cmems_fieldset(cs, deferLoadFlag=True, periodicFlag=False):
    ddir_head = cfg.ofam
    ddir = ddir_head
    files = sorted(glob(ddir+"ocean*.nc"))
    variables = {'U': 'u', 'V': 'v', 'W': 'w'}
    dimensions = {'lon': 'xu_ocean', 'lat': 'yu_ocean', 'depth': 'st_ocean', 'time': 'Time'}

    if cs not in ['auto', False]:
        # cs = (1, cs, cs) # == OLD initialization as tuple. Advised to rather use a dictionary. ==#
        cs = {dimensions['time']: 1, dimensions['depth']: cs,
              dimensions['lon']: cs, dimensions['lat']: cs}
    if periodicFlag:
        return FieldSet.from_netcdf(files, variables, dimensions, time_periodic=delta(days=30),
                                    deferred_load=deferLoadFlag, field_chunksize=cs)
    else:
        return FieldSet.from_netcdf(files, variables, dimensions, allow_time_extrapolation=True,
                                    deferred_load=deferLoadFlag, field_chunksize=cs)
