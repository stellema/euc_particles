...
plt.title('{} Particle distribution after {} years of advection ({}-{})'
          .format(lx['lb'][i], yrs, 1989, 1989-yrs, ddeg), loc='left')
m = Basemap(projection='merc', llcrnrlat=-16, urcrnrlat=16,\
...
for P in [t+100, t+200, t+10, 1+6]:
    # ITF, North, Recirc
    a, b = m(ds.lon[P].values, ds.lat[P].values) # ITF   
    m.scatter(a, b, s=5,  color=traj, zorder=10) # ITF 
...
#                  cax=fig.add_axes([0.91, 0.39425, 0.01915, 0.2]))
cbar = plt.colorbar(cs, orientation='vertical', 
                  cax=fig.add_axes([0.91, 0.3948, 0.01915, 0.175]))
cbar.ax.tick_params(labelsize=12, width=0.05)
cbar.set_label('Particles per bin', size=13)
fig.savefig(fpath.joinpath('{}_{}t_traj{}{}'.format(filename, yrs, ddeg, 
                           im_ext)), dpi=900, bbox_inches='tight')
plt.show(); plt.clf(); plt.close()