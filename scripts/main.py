...
def ofam_fieldset(time_bnds='full', exp='hist', chunks=300, add_xfields=True):
    ...
    if add_xfields:
        ...
        # xfiles = {'Ub': {'depth': mesh, 'lat': mesh, 'lon': mesh, 'data': xf},
        #           'Vb': {'depth': mesh, 'lat': mesh, 'lon': mesh, 'data': xf},
        #           'Wb': {'depth': mesh, 'lat': mesh, 'lon': mesh, 'data': xf},
        #           'Land': {'depth': mesh, 'lat': mesh, 'lon': mesh, 'data': xf},
        #           'zone': {'depth': zf, 'lat': zf, 'lon': zf, 'data': zf}}
        xfiles = {'Ub': xf,
                  'Vb': xf,
                  'Wb': xf,
                  'Land': xf,
...
#     if add_xfields:
#         # Add Unbeach velocity vectorfield to fieldset.
#         xfile = [str(cfg.data/'ofam_fields_unBL.nc')]
...
#         xfiles = {}
#         for n in xvars:
#             xdims[n] = {'lat': 'yu_ocean', 'lon': 'xu_ocean', 'depth': 'sw_ocean', 'time': 'Time'}
#             xfiles[n] = {'depth': mesh, 'lat': mesh, 'lon': mesh, 'data': xfile}
#             # xfiles[n] = xfile
#         # xdims['zone'] = {'lat': 'yu_ocean', 'lon': 'xu_ocean', 'time': 'Time'}
#         nmap = {"time": ["Time"],
#                 "lon": ["xu_ocean"],
#                 "lat": ["yu_ocean"],
#                 "depth": ["sw_ocean"]}
...
#         else:
#             zchunks = chunks
#         fieldsetUB = from_ofam(xfile, xvars, xdims,
#                                tracer_interp_method='bgrid_velocity',
#                                allow_time_extrapolation=True,
#                                field_chunksize=zchunks,
#                                chunkdims_name_map=nmap)
...
#         # UVWb = VectorField('UVWb', fieldset.Ub, fieldset.Vb, fieldset.Wb)
#         # fieldset.add_vector_field(UVWb)
#         zfile = [str(cfg.data/'ofam_zone_field.nc')]


#         zfield = Field.from_netcdf(zfile, 'zone', dims['U'],
