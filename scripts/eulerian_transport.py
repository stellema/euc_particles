...
# from main import paths, im_ext, idx, lx, width, height, LAT_DEG

years = cfg.lx['years']

# Open historical and future climatologies.
dh = xr.open_dataset(cfg.ofam/'ocean_u_{}-{}_climo.nc'.format(*years[0]))

dr = xr.open_dataset(cfg.ofam/'ocean_u_{}-{}_climo.nc'.format(*years[1]))
depth = dh.st_ocean[idx(dh.st_ocean, 450)].item()
...
for z in range(len(dz)):
    dh['u'][z] = dh.u.isel(st_ocean=z) * dz[z] * cfg.LAT_DEG * 0.1
    dr['u'][z] = dr.u.isel(st_ocean=z) * dz[z] * cfg.LAT_DEG * 0.1
...
plt.show()
plt.savefig(cfg.fig/'EUC_transport.png')
