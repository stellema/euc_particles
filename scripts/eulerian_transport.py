...
def llwbc_transport(exp=0, clim=False, sum_dims=['lon']):
    ...
    if clim:
        files = cfg.ofam / 'clim/ocean_v_{}-{}_climo.nc'.format(*cfg.years[exp])
    else:
...
def euc_transport(exp):
    ...
    """
    filename = cfg.data / 'transport_EUC_{}.nc'.format(cfg.exp_abr[exp])
    if filename.exists():
