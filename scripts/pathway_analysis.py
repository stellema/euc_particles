...
# q = 3
# where = ['min', 'mid','max', 'mid-min'][q]
# num_pids = [20, 5, 5, 4][q]  # Number of paths to plot (x3 for low/mid/high) per source.
...
for i, p in enumerate(pids):
    if where in ['min', 'mid','max']:
        ...
        c = ['mediumvioletred', 'b'][0 if i < num_pids else 1]
    dx = ds.sel(traj=p)
    ax.plot(dx.lon, dx.lat, c, linewidth=0.9, zorder=10, transform=proj,
            alpha=0.1)
    if lon == 165 and where != 'max':
        ax.set_extent([112, 220, -10, 10], crs=proj)

plt.tight_layout()
plt.savefig(cfg.fig / 'paths/dt_{}_r{}_z{}_{}_n{}.png'
