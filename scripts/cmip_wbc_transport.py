...
from cmip_fncs import (ofam_wbc_transport_sum, cmip_wbc_transport_sum, cmipMMM, cmip_wsc, sig_line)
...
def scatter_cmip_wbc_wsc(df, d6, d5):
    ...
    for i, cc in enumerate([mc, ng]):
        for x in [0, 2]:
            for dd, mip, dc in zip([d6, d5], [mip6, mip5], [dc6, dc5]):
                dd = dd[cc._n].mean('time')
                dc = dc.wsc.mean('lon').mean('time')
                for m, sym, symc in zip(mip.mod, mip.sym, mip.symc):
                    ...
                    if mip.mod[m]['id'] not in ['MIROC5', 'MIROC-ESM-CHEM', 'MIROC-ESM']:
                        ax[j].scatter(dd.isel(exp=x, model=m),
                                      dc.isel(exp=x, model=m, lat=i) * 1e7,
                                      color=symc, marker=MarkerStyle(sym, fillstyle='full'),
                                      label=mip.mod[m]['id'], s=mksize, linewidth=0.5)
            ax[j].axhline(y=0, color='grey', linewidth=0.6)  # Zero-line.
            ax[j].set_title('{}{} {} at {}'.format(cfg.lt[j], cfg.exps[x], cc.n, *cc._lat), loc='left')
            ax[j].set_xlabel('Transport [Sv]')
            if j == 1:
                ax[j].legend(bbox_to_anchor=(1, 1.125), loc="lower right", ncol=6, fontsize='small')
            elif j in [0, 2]:
                ax[j].set_ylabel('WSC [1e-7 N m-3]')
            j += 1
